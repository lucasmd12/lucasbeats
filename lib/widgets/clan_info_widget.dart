...
class _ClanInfoWidgetState extends State<ClanInfoWidget> {
  Clan? _clan;
  List<Member> _members = []; // Adicionado para armazenar os membros completos
  ...
  Future<void> _loadClanInfo() async {
    try {
      ...
      final members = await clanService.getClanMembers(widget.clanId); // Buscar membros completos

      if (mounted) {
        setState(() {
          _clan = clan;
          _members = members; // Atribuir membros completos
          _isLoading = false;
        });
      }
    } catch (e) {
              ...
              children: [
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    image: (_clan!.bannerImageUrl != null && _clan!.bannerImageUrl!.isNotEmpty)
                        ? DecorationImage(
                  ...
                  child: (_clan!.bannerImageUrl == null || _clan!.bannerImageUrl!.isEmpty)
                      ? Icon(
                          Icons.shield,
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${_clan!.members?.length ?? 0} membros', // Adicionado null check
                        style: TextStyle(
                          color: Colors.grey.shade400,
                          fontSize: 14,
                        ),
  ...
  Widget _buildClanStats() {
    final onlineMembersCount = _members.where((m) => m.isOnline).length;
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          _buildStatColumn(
            'Canais de Voz',
            '${_clan!.voiceChannels?.length ?? 0}', // Adicionado null check
            Icons.headset,
          ),
          _buildStatColumn(
            'Canais de Texto',
            '${_clan!.textChannels?.length ?? 0}', // Adicionado null check
            Icons.chat,
          ),
        ],
      ),
  ...
  Widget _buildOnlineMembers() {
    final onlineMembers = _members.where((m) => m.isOnline).toList();
        ...
        Wrap(
          ...
          children: onlineMembers.take(5).map((member) {
            return Container(
              ...
              child: Text(
                member.username, // Usando member.username
                style: const TextStyle(
                  color: Colors.white,
                  fontSize: 12,
                ),
