...
class _VoiceRoomWidgetState extends State<VoiceRoomWidget> {
  ...
  Widget build(BuildContext context) {
    return Consumer3<VoipService, FirebaseService, AuthService>(
      builder: (context, voipService, firebaseService, authService, child) {
                ...
                Row(
                  children: [
                    PermissionWidget(
                      ...
                      child: Expanded(
                        child: ElevatedButton.icon(
                          onPressed: _isCreatingRoom ? null : () => _createRoom(voipService, firebaseService, user),
                    ...
                    PermissionWidget(
                      ...
                      child: Expanded(
                        child: ElevatedButton.icon(
                          onPressed: () => _showJoinRoomDialog(voipService, firebaseService, user),
  ...
  bool _canCreateRoom(UserModel user) {
    switch (widget.roomType) {
      case 'clan':
 ...
 return user.role == Role.federationAdmin;
  ...
  bool _canJoinRoom(UserModel user) {
    switch (widget.roomType) {
      case 'clan':
 ...
 return user.role == Role.federationAdmin;
  ...
  Future<void> _createRoom(VoipService voipService, FirebaseService firebaseService, UserModel user) async {
    setState(() {
  ...
  void _showJoinRoomDialog(VoipService voipService, FirebaseService firebaseService, UserModel user) {
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton( // Aqui é onde você pode entrar em uma sala existente
              onPressed: () {
                final roomName = roomController.text.trim();
                if (roomName.isNotEmpty) {
                  Navigator.of(context).pop();
                  _joinRoom(voipService, firebaseService, user, roomName);
                }
              },
  ...
  Future<void> _joinRoom(VoipService voipService, FirebaseService firebaseService, UserModel user, String roomName) async {
    try {
      final displayName = user.username ?? 'Usuário Anônimo';
  ...
  String _generateRoomName(UserModel user) {
