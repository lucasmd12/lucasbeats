...
class _MembersTabState extends State<MembersTab> {
  ...
  Clan? _clan; // Variable to store clan details
 ...
 if (mounted) {
          setState(() {
            _error = 'Clã não encontrado ou erro ao carregar detalhes.';
            _isLoading = false;
          });
        }
 return; // Stop if clan details couldn't be loaded
      }

      // If clan details are fetched successfully, then fetch members
      final members = await clanService.getClanMembers(clanDetails.id);
      
      if (mounted) {
        setState(() {
 _clan = clanDetails; // Store clan details
          _members = members;
 ...
 break;
  ...
  void _transferLeadership(Member newLeader) async {
    try {
      // Add a confirmation dialog here if desired
      final clanService = Provider.of<ClanService>(context, listen: false);
      // Ensure clanId is not null before calling the service
      if (widget.clanId.isNotEmpty) {
        await clanService.transferLeadership(widget.clanId, newLeader.id);
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Liderança do clã transferida para ${newLeader.username} com sucesso!')),
        );
        _loadMembers(); // Refresh the list and clan details
      } else {
        Logger.error('Clan ID is empty, cannot transfer leadership.');
      }
    } catch (e) {
      Logger.error('Erro ao transferir liderança', error: e);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Erro ao transferir liderança: ${e.toString()}')),
 );
    }
  }
